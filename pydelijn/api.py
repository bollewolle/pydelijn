...
class Passages():
    ...
    async def get_passages(self):
        ...
        if resultstop is not None:
            stopname = "{}, {}".format(str(resultstop.get('omschrijving')),
            ...
            try:
                for index, passage in zip(range(self.maxpassages),
                    ...
                    if due_at_rt is not None:
                        dt_rt_local = tzone.localize(datetime.strptime(due_at_rt,
                                                                       "%Y-%m-%dT%H:%M:%S"),
                                                     is_dst=None)
                        ...
                        dt_rt_local = tzone.localize(datetime.strptime(due_at_sch,
                                                                       "%Y-%m-%dT%H:%M:%S"),
                                                     is_dst=None)
                        dt_now_local = tzone.localize(datetime.now(),
                                                      is_dst=None)
                    ...
                    endpointlinecolours = '{}lijnen/{}/{}/lijnkleuren'.format(BASE_URL,
                                                                              str(ent_num),
                                                                              str(linenumber))
